...
function ReceiveFriendMsg(CurrentQQ, data)
    ...
    if data.FromUin == tonumber(CurrentQQ) then -- 防止自我复读
        return 1
    end
    if string.find(data.Content, "链接转图") then
        ...
        response, error_message = http.request("GET", img_url, {
            query = "",
            headers = {
        ...
        if string.find(ContentType, "image") == 1 then
            ...
            CurrentQQ, {
                ...
                sendMsgType = "TextMsg", -- 进行文本复读回复
                groupid = 0, -- 不是私聊自然就为0咯
                content = "淦 老哥你的鸡掰地址好几把怪啊,类型不是图片诶它是" ..
                    ContentType, -- 回复内容
                atUser = 0 -- 是否 填上data.FromUserId就可以复读给他并@了
            })
...
function ReceiveGroupMsg(CurrentQQ, data)
    ...
    if string.find(data.Content, "链接转图") then
        ...
        local ContentType = "Content-Type" -- 减号是特殊字符 要么转义要么拐弯抹角 欢迎告诉我怎么转义变量名里的符号
        ContentType = re[ContentType]
        log.notice("img_url type--->  %s", ContentType)
        --[[for k, v in pairs(re) do --遍历结果测试用
					print(k, v)
				end]] --
        if string.find(ContentType, "image") == 1 then
